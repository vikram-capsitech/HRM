  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent className="overflow-hidden p-0 sm:max-w-lg [&>button:last-child]:hidden">
        <Command className="[&_[cmdk-group-heading]]:text-muted-foreground **:data-[slot=command-input-wrapper]:h-11 [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group]]:px-2 [&_[cmdk-input]]:h-11 [&_[cmdk-item]]:px-3 [&_[cmdk-item]]:py-2">
          {children}
        </Command>
      </DialogContent>
    </Dialog>
  ...
  return (
    ...
    >
      ...
      <CommandPrimitive.Input
        ...
        className={cn(
          "placeholder:text-muted-foreground/70 flex h-11 w-full rounded-md bg-transparent py-3 text-sm outline-hidden disabled:cursor-not-allowed disabled:opacity-50",
          className
        )}
        {...props}
      />
...
function CommandShortcut({
  ...
  return (
    <kbd
      ...
      className={cn(
        "bg-background text-muted-foreground/70 ms-auto -me-1 inline-flex h-5 text max-h-full items-center rounded border px-1 font-[inherit] text-[0.625rem] font-medium",
        className
      )}
      {...props}
    />
